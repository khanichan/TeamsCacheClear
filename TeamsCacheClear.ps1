...
if ($decision -eq 0) {
	...
	if (Get-Process | Where-Object {($teamsProcess.HasExited -or $outlookProcess.HasExited) -or ($teamsProcess -eq $null -or $outlookProcess -eq $null)}){
		#Wipe Teams Cache within multiple files in Appdata
		Get-ChildItem "C:\Users\*\AppData\Roaming\Microsoft\Teams\*" -directory |`
		Where name -in ('blob_storage','databases','GPUCache','IndexedDB','Local Storage','tmp','Cache','Local Storage','Code Cache') | `
		ForEach{Remove-Item $_.FullName -Recurse -Force -WhatIf}

		#Confirm in log its deleted
